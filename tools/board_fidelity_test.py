...
with sync_playwright() as p:
    ...
    st = pg.evaluate("""()=>{
      ...
      return {
        ...
        pipe:[].map.call(document.querySelectorAll('.stat-pipe .stat .pn'),function(x){return x.textContent;}),
        chips:[].map.call(document.querySelectorAll('.stat-flags .stat'),function(x){return x.getAttribute('aria-label')||'';}).join(' | '),
        pills:[].map.call(document.querySelectorAll('.stat-flags .stat'),function(x){return x.getAttribute('aria-label')||'';}).join(' | '),
