...
function rosterInnerHtml(){
  ...
  return __roster.map(function(m, i){
    // The member LABEL preference: the human display_name (from console_profiles), then the email (already in
    // the console_members roster read), then a short "unnamed" label. The raw uuid is NEVER shown as a name:
    // a member with no display_name is a person, not a hex string, so we fall back to the address we already
    // have rather than to m.uid. m.name here is console_profiles.display_name, never console_members.uid.
...
// console_members_read_owner); ONLY id/email are used, never role. The human NAME and the current title both
// come from console_profiles (display_name + signature_title) in the SAME bounded read, so there is no new
// fetch. A member with no display_name is labelled by their email, never their uid (console_members.name can
// itself hold the raw uid, so it is deliberately not used as the label). A member with no profile row, or a
// row the reader cannot see, shows a blank name (email/unnamed fallback covers it) and a blank title, never a
// crash. Both reads are bounded and best-effort (identGet over authFetchOnce).
function loadRoster(){
  if(!isOwner()) return Promise.resolve();
  return identGet("console_members?select=id,name,email&order=name.asc").then(function(mrows){
    var members = (mrows||[]).map(function(r){ return { uid:(r&&r.id)||"", email:(r&&r.email)||"" }; })
                             .filter(function(m){ return m.uid; });
    return identGet("console_profiles?select=uid,display_name,signature_title").then(function(prows){
      var nameByUid = {}, titleByUid = {};
      (prows||[]).forEach(function(p){ if(p && p.uid){ nameByUid[p.uid] = String(p.display_name||""); titleByUid[p.uid] = String(p.signature_title||""); } });
      __roster = members.map(function(m){ m.name = nameByUid[m.uid] || ""; m.title = titleByUid[m.uid] || ""; return m; });
      var host=document.getElementById("admRoster"); if(host){ host.innerHTML = rosterInnerHtml(); wireRoster(); }
    }, function(){                                             // profiles unreadable: still render, blank name + title
      __roster = members.map(function(m){ m.name=""; m.title=""; return m; });
      var host=document.getElementById("admRoster"); if(host){ host.innerHTML = rosterInnerHtml(); wireRoster(); }
    });
