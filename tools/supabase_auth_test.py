...
# P47 restore: rest still sends the session bearer when signed in, and bearer() is the last-good (P31)
# form: the session JWT when present, otherwise the anon key. The P39 sign-in flag and the kind:"session"
# throw were removed on purpose (the P47 build diff proved the freeze was not on this path).
ck("rest sends the session bearer, not always the anon key",
   '"Authorization": "Bearer " + bearer()' in sup and "s && s.access_token" in sup
   and "(s && s.access_token) ? s.access_token : c.anon" in sup)
_bstart = sup.find("function bearer(")
_bbody = sup[_bstart:_bstart + 300] if _bstart >= 0 else ""
ck("P47: bearer() is the last-good form (session JWT else anon) and never throws a session error",
   bool(_bbody) and "__signInSeen" not in _bbody and 'e.kind = "session"' not in _bbody
   and "(s && s.access_token) ? s.access_token : c.anon" in _bbody)
