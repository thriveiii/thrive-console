...
REC_ABOVE_SEND = """()=>{ var dw=document.getElementById('owModeA'); var rec=document.getElementById('recIn'); var snd=dw&&dw.querySelector('#nmSend'); if(!rec||!snd) return false; return (rec.compareDocumentPosition(snd) & Node.DOCUMENT_POSITION_FOLLOWING)!==0; }"""
...
with sync_playwright() as p:
    ...
    pg.evaluate(OPEN, "Alpha Co"); pg.wait_for_selector("#owModeA #edSubj", timeout=6000); pg.wait_for_timeout(600)  # > detail fetch
    ...
    pg.evaluate(OPEN, "Alpha Co"); pg.wait_for_selector("#owModeA #edSubj", timeout=6000); pg.wait_for_timeout(600)
    ...
    pg.evaluate(OPEN, "Beta LLC"); pg.wait_for_selector("#owModeA #edSubj", timeout=6000); pg.wait_for_timeout(600)
    ...
    pg.evaluate(OPEN, "Gamma Inc"); pg.wait_for_selector("#owModeA #edSubj", timeout=6000); pg.wait_for_timeout(600)
    ...
    pg.evaluate(OPEN, "Beta LLC"); pg.wait_for_selector("#owModeA #edSubj", timeout=6000); pg.wait_for_timeout(600)
    arr = pg.evaluate("""()=>{ var dw=document.getElementById('owModeA'); return { dir:dw?getComputedStyle(dw).direction:'', save:dw?dw.textContent.indexOf('حفظ المستلم')>=0:False, hasField:!!document.getElementById('recIn') }; }""")
    ck("5: AR flips the window compose to RTL and the recipient field is present", arr["dir"]=="rtl" and arr["hasField"], arr)
